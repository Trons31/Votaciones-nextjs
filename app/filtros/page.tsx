...
export default async function FiltrosPage({
  ...
  const totalColegio =
    colegio
      ? await prisma.voter.count({ where: { ...leaderWhere, dondeVota: colegio } })
      : null;
  ...
  let porLider:
    | { independentes: number; rows: Array<{ label: string; count: number }> }
    | null = null;
  ...
  if (leaderFilter.kind === "all") {
    ...
    porLider = {
      ...
      rows: grouped
        .map((g) => ({ label: map.get(g.leaderId!) || `Líder ${g.leaderId}`, count: g._count._all }))
        .filter((r) => r.count > 0)
    };
  }
  ...
  const maxLiderCount =
    porLider && porLider.rows.length > 0
      ? Math.max(...porLider.rows.map((r) => r.count), porLider.independentes)
      : 1;
  ...
  function buildHref(p: number) {
    ...
    return `/filtros?${params.toString()}`;
  }
      ...
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <div className="rounded-xl bg-white/80 p-6 shadow-lg backdrop-blur-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-3xl font-bold tracking-tight text-slate-900">Filtros</h1>
              <p className="text-sm text-slate-600">Visualiza y exporta datos filtrados de votantes</p>
            </div>
          </div>
        </div>
                  ...
                  return (
                    <div key={r.dondeVota || idx}>
                      ...
                      <div className="h-3 w-full overflow-hidden rounded-full bg-slate-100">
                        <div className="h-full rounded-full bg-gradient-to-r from-indigo-500 to-indigo-600 transition-all duration-500" style={{ width: `${width}%` }} />
                      </div>
                    </div>
                  );
                }) : <div className="py-8 text-center text-sm text-slate-500">Sin datos disponibles</div>
              ) : porMesa.length > 0 ? porMesa.map((r, idx) => {
                const width = (r._count._all / maxMesaCount) * 100;
                return (
                  <div key={r.mesaVotacion || idx}>
                    <div className="mb-1 flex items-center justify-between text-sm">
                      <span className="font-medium text-slate-700">Mesa {r.mesaVotacion || "(Sin mesa)"}</span>
                      <span className="ml-2 font-bold text-indigo-600">{r._count._all}</span>
                    </div>
                    <div className="h-3 w-full overflow-hidden rounded-full bg-slate-100">
                      <div className="h-full rounded-full bg-gradient-to-r from-indigo-500 to-indigo-600 transition-all duration-500" style={{ width: `${width}%` }} />
          ...
          <div className="rounded-xl bg-white/80 p-6 shadow-lg backdrop-blur-sm">
            ...
            {leaderFilter.kind !== "all" ? (
              <div className="flex flex-col items-center justify-center py-12 text-center">
                ...
                <p className="mt-3 text-sm text-slate-600">Selecciona "Todos los líderes" para ver esta distribución</p>
              </div>
            ) : porLider ? (
              <div className="space-y-3">
                <div>
                  ...
                  <div className="h-3 w-full overflow-hidden rounded-full bg-slate-100">
                    <div className="h-full rounded-full bg-gradient-to-r from-slate-400 to-slate-500 transition-all duration-500" style={{ width: `${(porLider.independentes / maxLiderCount) * 100}%` }} />
                  </div>
                </div>
                {porLider.rows.slice(0, 10).map((r, idx) => (
                  <div key={idx}>
                    ...
                    <div className="h-3 w-full overflow-hidden rounded-full bg-slate-100">
                      <div className="h-full rounded-full bg-gradient-to-r from-purple-500 to-purple-600 transition-all duration-500" style={{ width: `${(r.count / maxLiderCount) * 100}%` }} />
                    </div>
                  </div>
                ))}
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-600">
                    Mostrando {(page - 1) * PAGE_SIZE + 1}–{Math.min(page * PAGE_SIZE, totalFiltered)} de {totalFiltered.toLocaleString()} registros
                  </p>
                </div>
              </div>
            </div>
                ...
                <tbody className="divide-y divide-slate-100 bg-white">
                  {voters.map((v) => (
                    <tr key={v.id} className="transition-colors hover:bg-slate-50">
                      ...
                      <td className="px-4 py-3">
                        <span className="font-medium text-slate-900">{v.apellidos}, {v.nombres}</span>
                      </td>
  ...
  return (
    <div className="mt-4 flex flex-col items-center justify-between gap-3 sm:flex-row">
      <p className="text-xs text-slate-500">
        ...
        <span className="font-semibold text-slate-700">{total.toLocaleString()}</span> votantes
      </p>
      ...
      <div className="flex items-center gap-1">
        ...
        {pages.map((p, i) =>
          p === "…" ? (
            ...
            <span
              key={p}
              className="inline-flex h-8 w-8 items-center justify-center rounded-md bg-indigo-600 text-xs font-semibold text-white shadow-sm"
            >
              {p}
            </span>
          ) : (
